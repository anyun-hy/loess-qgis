...
LOESS_LAUNCHER_CONFIG="${LOESS_RUNTIME_ROOT}/loess_launcher.sh"

LOESS_CONFIGURED_PLATFORM=""
LOESS_CONFIGURED_CONDA_EXE=""
LOESS_CONFIGURED_CONDA_ENV=""
LOESS_CONDA_EXE_PINNED=0
if [ -f "${LOESS_LAUNCHER_CONFIG}" ]; then
  # Generated by init_project.sh with shell-quoted scalar values.
  source "${LOESS_LAUNCHER_CONFIG}"
  CONDA_ENV="${LOESS_CONDA_ENV_OVERRIDE:-${LOESS_CONFIGURED_CONDA_ENV}}"
  CONDA_EXE="${LOESS_CONDA_EXE_OVERRIDE:-${LOESS_CONFIGURED_CONDA_EXE}}"
  LOESS_PLATFORM="${LOESS_PLATFORM_OVERRIDE:-${LOESS_CONFIGURED_PLATFORM}}"
  if [ -n "${CONDA_EXE}" ]; then
    LOESS_CONDA_EXE_PINNED=1
  fi
else
...
if [ "${LOESS_CONDA_EXE_PINNED}" -eq 1 ] && [ ! -x "${CONDA_EXE}" ]; then
  echo "Configured Conda executable is not executable: ${CONDA_EXE}" >&2
  return 2 2>/dev/null || exit 2
fi

if [ "${LOESS_CONDA_EXE_PINNED}" -eq 0 ] && [ ! -x "$CONDA_EXE" ]; then
  CONDA_EXE=""
